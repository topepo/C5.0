...
model.frame.C5.0 <- function(formula, ...) {
  ...
  mf <- mf[c(
    1L,
    match(
      c("formula", "data", "subset", "na.action", "weights"),
      names(mf),
      0L
    )
  )]
...
as.party.C5.0 <- function(obj, trial = 0, ...) {
  ...
  if (!is.default) {
    ...
    if (is.na(wspot)) {
      mf <-
        data.frame(
          x = eval(parse(text = paste(obj$call)[xspot])),
          y = eval(parse(text = paste(obj$call)[yspot]))
        )
      names(mf) <- c(obj$pred, "y")
    } else {
      mf <-
        data.frame(
          eval(parse(text = paste(obj$call)[xspot])),
          eval(parse(text = paste(obj$call)[yspot])),
          eval(parse(text = paste(obj$call)[wspot]))
        )
  ...
  if (length(out) == 1) {
    ...
    n.cat <- sapply(1:length(obj$pred), function(i) {
      is.factor(mf[, obj$pred[i]])
    })
    adj.pred <- as.vector(sapply(obj$pred, function(i) {
      gsub("`", "", i)
    }))
  ...
  if (is.default) {
    ...
    g7 <- party(pn, data = mf[0L, ], fitted = dat1, terms = terms(fn))
  } else {
