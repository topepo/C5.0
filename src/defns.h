...
#define Swap(a, b)                                                             \
  {                                                                            \
    DataRec xab;                                                               \
    assert(a >= 0 && a <= MaxCase && b >= 0 && b <= MaxCase);                  \
    xab = Case[a];                                                             \
    Case[a] = Case[b];                                                         \
    Case[b] = xab;                                                             \
  }
...
#define RelCWt(Case)                                                           \
  (Unknown(Case, CWtAtt) || NotApplic(Case, CWtAtt) || CVal(Case, CWtAtt) <= 0 \
       ? 1                                                                     \
...
void DayToDate(int DI, String Date, size_t DS_size);
int TimeToSecs(String TS);
void SecsToTime(int Secs, String Time, size_t DS_size);
