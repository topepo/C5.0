...
/*   --------------------  */
{
  ...
  if (!XVAL) {
    if (!RULES) {
      ForEach(Trial, 0, TRIALS - 1) { SaveTree(Pruned[Trial], ".tree"); }
    } else {
      ForEach(Trial, 0, TRIALS - 1) { SaveRules(RuleSet[Trial], ".rules"); }
    }
...
/*   --------------  */
{
  ...
  ForEach(i, 0, MaxCase) {
    ...
    if (PredClass != RealClass) {
      Errs++;
      if (MCost)
        ECost += MCost[PredClass][RealClass];
    }
  ...
  if (RULES) {
    fprintf(Of, "  %4d %4d(%4.1f%%)", RuleSet[0]->SNRules, Errs,
            100 * Errs / Tests);
  } else {
    ...
    Verbosity(1,
              {
                fprintf(Of, "  %4d %4d(%4.1f%%)  ", TreeSize(Raw[0]), RawErrs,
                        100 * RawErrs / Tests);
              })

        /*  Results for pruned tree  */

        fprintf(Of, "  %4d %4d(%4.1f%%)", TreeSize(Pruned[0]), Errs,
                100 * Errs / Tests);
  }
  ...
  if (UtilErr) {
    if (!XVAL) {
      fprintf(Of,
              ...
              (MCost ? "   " F_Cost : ""), (MCost ? "   " F_UCost : ""));

      ForEach(u, 1, UTILITY - 1) {
        fprintf(Of, "\t%s%d\t %4d(%4.1f%%)", (UtilBand[u] == 1 ? "" : "1-"),
                UtilBand[u], UtilErr[u], 100 * UtilErr[u] / Tests);
        if (MCost) {
...
/*   -------------  */
{
  ...
  ForEach(i, 0, MaxCase) {
    ...
    if (PredClass != RealClass) {
      BoostErrs++;
      if (MCost)
        BoostECost += MCost[PredClass][RealClass];
    }
    ...
    ForEach(t, 0, TRIALS - 1) {
      if (TrialPred[t] != RealClass) {
        Errs[t]++;
        if (MCost)
          ECost[t] += MCost[TrialPred[t]][RealClass];
      }
    }
  }
  ...
  ForEach(t, 0, TRIALS - 1) {
    ...
    if (RULES) {
      fprintf(Of, "  %4d %4d(%4.1f%%)", RuleSet[t]->SNRules, Errs[t],
              100 * Errs[t] / Tests);
    } else {
      fprintf(Of, "  %4d %4d(%4.1f%%)", TreeSize(Pruned[t]), Errs[t],
              100 * Errs[t] / Tests);
    }
  ...
  if (RULES) {
    fprintf(Of, F_Boost "\t  %9d(%4.1f%%)", BoostErrs, 100 * BoostErrs / Tests);
  } else {
    fprintf(Of, F_Boost "\t       %4d(%4.1f%%)", BoostErrs,
            100 * BoostErrs / Tests);
  }
