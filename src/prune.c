...
First record possible values for discrete attributes  */
  ...
  ForEach(Att, 1, MaxAtt) {
    if (Ordered(Att) || (Discrete(Att) && SUBSET)) {
      ...
      ForEach(i, 1, MaxAttVal[Att]) { SetBit(i, PossibleValues[Att]); }
    }
  }
...
/*   ------------  */
{
  ...
  CaseCount Cases = 0, KnownCases, *BranchCases, MissingCases, *SmallBranches,
            SmallBranchCases = 0, Errs, SaveErrs, TreeErrs, LeafErrs,
            ExtraLeafErrs = 0, BestBrErrs;
  ...
  if (!T->NodeType) /*  leaf  */
  {
    if ((Flags & UPDATE) && (Flags & REPORTPROGRESS) && Now == SIMPLIFYTREE &&
        T->Cases > 0) {
      Progress(T->Cases);
    }
  ...
  if (SmallBranchCases) {
    ...
    ForEach(c, 2, MaxClass) {
      if (SmallBranches[c] > SmallBranches[BestClass])
        BestClass = c;
    }
...
This test is skipped for boosted trees  */
  ...
  ForEach(v, 1, T->Forks) {
    if (!T->Branch[v]->NodeType || T->Branch[v]->Cases < 0.1 * T->Cases ||
  ...
  LocalVerbosity(
      ...
      fprintf(Of, "%s:  [%d%%  N=%.2f  tree=%.2f  leaf=%.2f+%.2f",
              AttName[T->Tested], (int)((TreeErrs * 100) / (T->Cases + 0.001)),
...
directly by checking training data  */
  ...
  while (MaxExtraErrs > 0) {
    ...
    Verbosity(2, if (NXT > 0 && TotalExtraErrs > MaxExtraErrs) fprintf(
                     Of, "%d tied with MinCC=%.3f; total extra errs %.1f\n",
    ...
    ForEach(x, 0, NXT - 1) {
      ...
      Verbosity(2, fprintf(Of, "global: %d leaves, %.1f errs\n", DeltaLeaves,
                           DeltaErrs))
    }
...
/*   ------------  */
{
  ...
  if (T->NodeType == BrSubset) {
    ...
    if (!Ordered(A)) {
      ...
      for (v = First; v < T->Forks; v++) {
        ...
        for (vv = v + 1; vv <= T->Forks;) {
          if (!T->Branch[vv]->NodeType && T->Branch[vv]->Leaf == LeafBr->Leaf &&
           ...
           or replace if branch v has no cases  */
              ...
              ForEach(c, 1, MaxClass) {
                if (!LeafBr->Cases) {
                  LeafBr->ClassDist[c] = T->Branch[vv]->ClassDist[c];
                } else {
                  LeafBr->ClassDist[c] += T->Branch[vv]->ClassDist[c];
                }
              }
            ...
            ForEach(b, 0, Bytes - 1) { T->Subset[v][b] |= T->Subset[vv][b]; }
    ...
    ForEach(v, 1, T->Forks) { CheckSubsets(T->Branch[v], PruneDefaults); }
  }
}
...
float Val[] = {0, 0.001, 0.005, 0.01, 0.05, 0.10, 0.20, 0.40, 1.00},
      Dev[] = {4.0, 3.09, 2.58, 2.33, 1.65, 1.28, 0.84, 0.25, 0.00}, Coeff;
...
/*   ---------------  */
{
  ...
  if (T->NodeType) {
    ForEach(v, 1, T->Forks) { RestoreDistribs(T->Branch[v]); }
  }
